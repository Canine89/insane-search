...
<!-- Codex port note: this plugin was originally packaged for Claude Code. -->
**Step 0 — Codex runtime setup:** when running engine commands from Codex, create and use a plugin-local virtualenv from the skill directory:

```bash
# Run from the directory containing this SKILL.md.
PLUGIN_ROOT="$(cd ../.. && pwd)"
python3 -m venv "$PLUGIN_ROOT/.venv"
"$PLUGIN_ROOT/.venv/bin/python" -m pip install -q -U pip
"$PLUGIN_ROOT/.venv/bin/python" -m pip install -q "curl_cffi>=0.15.0" beautifulsoup4 pyyaml yt-dlp
"$PLUGIN_ROOT/.venv/bin/python" -m engine "<URL>" [--selector "<CSS>"] [--device auto|desktop|mobile] [--trace]
```

Do not run the Claude-only `AskUserQuestion` or GitHub star prompt flow.
